...
import {
  CameraCapture,
  CameraCaptureHandle,
} from "@/features/verification/components/verification/CameraCapture";
import { FaceGuideOverlay } from "@/features/verification/components/verification/FaceGuideOverlay";
import { ResultManualReview } from "@/features/verification/components/verification/ResultManualReview";
import { ResultRejected } from "@/features/verification/components/verification/ResultRejected";
import { useVerificationMachine } from "@/features/verification/hooks/useVerificationMachine";
import { Vote } from "lucide-react";
import React, { useEffect, useRef, useState } from "react";
import {
  useFaceVerification,
  useFaceVerificationMutationType,
} from "../../api/use-face-verification";
import { VerifyApiResponse } from "@/lib/verification/types";
import { useFaceDetection } from "@/features/verification/hooks/useFaceDetection";
import { useElectionId } from "../../hooks/use-get-election-id";
import { FaceResultApproved } from "./result-approved";
import { useVotingContext } from "@/context/voting-context";

interface FaceVerificationProps {
  sessionId: string;
}
const FaceVerification = ({ sessionId }: FaceVerificationProps) => {
  const {
    state,
    dispatch,
    requestCamera,
  ...
  const { setVotinData } = useVotingContext();
  ...
  async function handleSubmit() {
    ...
    try {
      mutate(
        ...
        {
          onSuccess: (data) => {
            // const res = data as useFaceVerificationMutationType
            const result = data.data as VerifyApiResponse;
            console.log("result ", result);

            if (result.token && result.expiresAt) {
              console.log("token ", result.token);
              setVotinData(result.token, result.expiresAt, sessionId);
            }
  ...
  return (
    <div className="flex w-full flex-col gap-6 p-8">
      <div className="flex items-start gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            We have to verify the user before you can vote.
          </p>
        </div>
      </div>
      ...
      {state.status === "approved" && (
        <FaceResultApproved similarity={state.similarity} />
      )}
